...
class TestFullUserJourney:
    ...
    def mock_ai_service(self, monkeypatch):
        ...
        monkeypatch.setattr(
            openrouter.OpenRouterService, "generate_critique", mock_generate_critique
        )
    ...
    def test_complete_user_journey(self, e2e_client, mock_ai_service, test_image, supabase_admin):
        ...
        try:
            ...
            # Verify features were extracted and stored
            # Note: individual attribute scores are stored in model_scores JSONB,
            # not exposed directly in the response. features_json contains scene context.
            assert (
                photo_data.get("features_json") is not None
                or photo_data.get("scene_type") is not None
            )
